...
class HistoryEventsDeletionSchema(HistoryEventFilterSchema):
    ...
    def validate_deletion_schema(
        ...
        for field_name, field_obj in self.fields.items():
            ...
            if field_name == 'to_timestamp':
                # to_timestamp has a default value (current time), so we check if it was
                # explicitly provided by the user rather than comparing against the default
                if field_name in self._explicitly_provided_fields:
                    has_filter = True
                    break
                continue

            if (
                (isinstance(field_obj, fields.Boolean) and not default and value) or
