...
def edit_token_and_clean_cache(
    ...
    if updated_fields is True:
        if isinstance(token, EvmToken):
            GlobalDBHandler.edit_evm_token(token)  # type: ignore[arg-type]
        else:
            GlobalDBHandler.edit_solana_token(token)  # type: ignore[arg-type]
...
def _get_or_create_token(
    ...
    with userdb.get_or_create_token_lock:
        ...
        if None in (name, symbol, decimals, token_kind) and chain_inquirer is not None:
            ...
            if None in (name, symbol, decimals, token_kind) and None not in (fallback_name, fallback_symbol, fallback_decimals):  # noqa: E501
